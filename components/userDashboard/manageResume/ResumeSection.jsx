...
function Index(props) {
  const { title = '', experiences = [], allowEdit = false, allowXpEdit } = props || {};

  const classes = useStyles({cursor: allowEdit && 'pointer'});

  const editResumeSection = (e) => {
    e.stopPropagation();
    props.setCurrentResumeSectionItemId(props.id);
    props.setShowResumeSectionFormModal(true);
  };

  return (
    <div onClick={props.onClick}>
      {/* <TouchRipple> */}
      <Paper className={`with-edit-on-hover ${classes.paper}`}>
        {
          allowEdit && (
            <IconButton aria-label="close" className={`edit-on-hover ${classes.edit} ${classes.small}`} onClick={editResumeSection}>
